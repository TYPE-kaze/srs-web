<script>
	...
	const data = $derived.by(() => {
		...
		for (let i = 1; i < numOfDay; i++) {
			...
			const obj = { Ngày: date, "Số kiến thức": 0 };
			mapping[fromDateToDateID(date)] = obj;
			ret.push(obj);
		}

		for (const k of kArr) {
			const key = fromDateToDateID(k.due);
			if (mapping[key]) {
				mapping[key]["Số kiến thức"]++;
			}
		}
		return ret;
	});
	const listing = $derived.by(() => {
		...
		let total = data.reduce((a, d) => a + d["Số kiến thức"], 0);
		value = `${total} kiến thức`;
		ret.push({ id: "total", label: "Tổng cộng", value });

		value = total / numOfDay;

		value = `${value.toFixed(2)} kt/ngày`;
...
<div class="container-fluid bg-opacity-10 border-secondary-subtle bg-body-tertiary rounded border shadow-sm">
	<div class="d-flex border-bottom border-secondary py-2">
		<h3 class="m-0">Số lượng sẽ ôn tập</h3>
